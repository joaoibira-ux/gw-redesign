...
const TOOLS_GW = [
  ...
  },
  {
    ...
    description: "Exclui PERMANENTEMENTE um lançamento do Contas a Pagar (remove o documento — não é reversível). Use consultar_contas_pagar ANTES pra obter o id real (NUNCA invente um id como '1', '2', '3' — só o id exato retornado por consultar_contas_pagar é válido) e confirme com o usuário a descrição e o valor do lançamento antes de excluir, deixando claro que é definitivo. Se o usuário só quer marcar como pago (não excluir de vez), use dar_baixa_conta_pagar. ALTERA O BANCO DE DADOS: exige senha de autorização, peça ao usuário antes de chamar.",
    input_schema: {
      type: "object",
      properties: {
        id:    { type: "string", description: "ID do lançamento (obtido via consultar_contas_pagar — nunca invente)" },
...
async function executarFerramenta(nome, input, apiKeyValue) {
  ...
  if (nome === "excluir_conta_pagar") {
    ...
    return {
      sucesso: true,
      mensagem: "Lançamento excluído permanentemente do Contas a Pagar.",
      id,
