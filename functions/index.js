...
const TOOLS_GW = [
  ...
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: {
        funcionarioNome: { type: "string", description: "Nome (parcial ou completo) do funcionário cujos adiantamentos serão mostrados" },
        destinatarioNome: { type: "string", description: "Nome (parcial ou completo) do funcionário que vai RECEBER a mensagem no WhatsApp — o telefone é buscado automaticamente" },
        apenasPagos: { type: "boolean", description: "Se true, mostra só os adiantamentos já descontados da folha (quitados). Se omitido ou false (padrão), mostra só os ainda em aberto." }
      },
      required: ["funcionarioNome", "destinatarioNome"]
    }
  },
  {
    name: "criar_contato",
  ...
  },
  {
    ...
    description: "Anexa o PDF de um boleto recebido pelo WhatsApp a um lançamento JÁ EXISTENTE no Contas a Pagar, em vez de criar um lançamento novo (pra criar um novo, use registrar_boleto_contas_pagar). Use consultar_contas_pagar antes para listar os lançamentos em aberto e obter o id correto (NUNCA invente um id) — mostre a lista ao usuário e confirme qual lançamento é antes de aplicar. Se o vencimento do boleto (data) for diferente da data que o lançamento já tinha, a data do lançamento é atualizada para a do boleto automaticamente. O campo boletoUrl deve ser exatamente o valor recebido na mensagem do sistema (nunca invente, altere ou omita essa URL). ALTERA O BANCO DE DADOS: exige senha de autorização, peça ao usuário antes de chamar.",
    input_schema: {
      type: "object",
      properties: {
        id:                { type: "string", description: "ID do lançamento já existente (obtido via consultar_contas_pagar)" },
        data:              { type: "string", description: "Data de vencimento do boleto, DD/MM/AAAA, confirmada ou corrigida pelo usuário. Se vier diferente da data atual do lançamento, o lançamento é atualizado pra essa data." },
        boletoUrl:         { type: "string", description: "URL permanente do PDF do boleto no Cloud Storage — repassar EXATAMENTE como recebida na mensagem do sistema, nunca alterar." },
        boletoNomeArquivo: { type: "string", description: "Nome original do arquivo PDF (opcional)." },
        senha:             { type: "string", description: "Senha de autorização para alterar o banco de dados. Deve ser pedida ao usuário antes de chamar esta ferramenta." }
      },
  ...
  },
  {
    ...
    description: "Cria um novo lançamento no Contas a Receber. ALTERA O BANCO DE DADOS: exige o campo senha, que deve ser pedido ao usuário antes de chamar esta ferramenta.",
    input_schema: {
      type: "object",
      properties: {
        data:      { type: "string", description: "Data do lançamento no formato DD/MM/YYYY" },
        descricao: { type: "string", description: "Descrição do lançamento (ex: nome de quem vai pagar)" },
        valor:     { type: "number", description: "Valor em reais" },
        senha:     { type: "string", description: "Senha de autorização para alterar o banco de dados. Deve ser pedida ao usuário antes de chamar esta ferramenta." }
      },
      required: ["data", "descricao", "valor", "senha"]
    }
  },
  {
    name: "editar_conta_receber",
    description: "Edita a descrição, o valor e/ou a data de um lançamento já existente no Contas a Receber. Use consultar_contas_receber antes para obter o id correto (NUNCA invente um id como '1', '2', '3' — só o id exato retornado por consultar_contas_receber é válido) e confirme com o usuário qual lançamento é (descrição e valor atuais) antes de aplicar a alteração. Pra marcar como recebido, prefira dar_baixa_conta_receber (mais direto). ALTERA O BANCO DE DADOS: exige senha de autorização, peça ao usuário antes de chamar.",
    input_schema: {
      type: "object",
      properties: {
        id:        { type: "string", description: "ID do lançamento (obtido via consultar_contas_receber)" },
        descricao: { type: "string", description: "Nova descrição (opcional, mantém a atual se omitido)" },
        valor:     { type: "number", description: "Novo valor (opcional, mantém o atual se omitido)" },
        data:      { type: "string", description: "Nova data DD/MM/YYYY (opcional, mantém a atual se omitido)" },
        senha:     { type: "string", description: "Senha de autorização para alterar o banco de dados. Deve ser pedida ao usuário antes de chamar esta ferramenta." }
  ...
  {
    name: "dar_baixa_conta_receber",
    description: "Marca UM lançamento do Contas a Receber como recebido (status 'baixado'). Use quando o usuário pedir pra 'dar baixa', 'marcar como recebido' ou 'quitar' um lançamento. Use consultar_contas_receber ANTES pra obter o id real (NUNCA invente um id como '1', '2', '3' — só o id exato retornado por consultar_contas_receber é válido) e confirme com o usuário qual lançamento é (descrição e valor) antes de aplicar. Se o usuário pedir baixa em MAIS DE UM lançamento, chame esta ferramenta UMA VEZ PARA CADA um deles, com o id real de cada — nunca responda como se tivesse dado baixa em algo sem ter chamado a ferramenta pra aquele item específico. ALTERA O BANCO DE DADOS: exige senha de autorização, peça ao usuário antes de chamar.",
    input_schema: {
      type: "object",
      properties: {
        id:    { type: "string", description: "ID do lançamento (obtido via consultar_contas_receber — nunca invente)" },
        senha: { type: "string", description: "Senha de autorização para alterar o banco de dados. Deve ser pedida ao usuário antes de chamar esta ferramenta." }
      },
      required: ["id", "senha"]
    }
  },
  {
    ...
    description: "Exclui um lançamento do Contas a Receber já existente. Use consultar_contas_receber antes para obter o id correto e confirme com o usuário qual lançamento é (descrição e valor) antes de excluir. ALTERA O BANCO DE DADOS: exige o campo senha, que deve ser pedido ao usuário antes de chamar esta ferramenta.",
    input_schema: {
      type: "object",
      properties: {
        id:    { type: "string", description: "ID do lançamento (obtido via consultar_contas_receber)" },
...
// Converte o campo livre "telefone" de um funcionário (ex: "(81) 99211-4764")
// pro formato que a Evolution API espera: só dígitos, com DDI 55 na frente.
...
async function executarFerramenta(nome, input, apiKeyValue) {
  ...
  if (nome === "enviar_extrato_adiantamentos_whatsapp") {
    const { funcionarioNome, destinatarioNome, apenasPagos } = input;
    const dados = await buscarExtratoAdiantamentosFuncionario(funcionarioNome, !!apenasPagos);
    ...
    return {
      sucesso: true,
      mensagem: `Imagem enviada por WhatsApp para ${dest.nome} e em cópia para o número fixo do responsável.`,
      funcionarioNome: dados.funcionarioNome,
  ...
  if (nome === "anexar_boleto_conta_pagar") {
    const { id, data, boletoUrl, boletoNomeArquivo, senha } = input;
    if (senha !== SENHA_ALTERACAO_BANCO) {
      return { sucesso: false, erro: "senha_invalida", mensagem: "Senha incorreta. Peça a senha de autorização ao usuário para alterar o banco de dados." };
    }
    if (!boletoUrl) {
    ...
    const dataAlterada = data && data !== atual.data;
    if (dataAlterada) updates.data = data;
    await ref.update(updates);

    return {
      sucesso: true,
      mensagem: dataAlterada
        ? `Boleto anexado ao lançamento existente no Contas a Pagar. Data atualizada de ${atual.data} para ${data} (vencimento do boleto).`
        : "Boleto anexado ao lançamento existente no Contas a Pagar.",
      id,
      descricao: atual.descricao,
      valor: atual.valor,
      dataAnterior: atual.data,
      dataAtual: dataAlterada ? data : atual.data
    };
  }
  ...
  if (nome === "criar_conta_receber") {
    const { data, descricao, valor, senha } = input;
    if (senha !== SENHA_ALTERACAO_BANCO) {
      return { sucesso: false, erro: "senha_invalida", mensagem: "Senha incorreta. Peça a senha de autorização ao usuário para alterar o banco de dados." };
    }
    if (!data || !descricao || !valor) {
      return { sucesso: false, erro: "parametros_invalidos", mensagem: "data, descricao e valor são obrigatórios." };
    }

    const refContaReceber = db.collection("contasReceber").doc();
    await refContaReceber.set({
      data,
      descricao,
      valor: Number(valor) || 0,
      status: "aberto",
    ...
    return {
      sucesso: true,
      mensagem: "Lançamento criado no Contas a Receber.",
      contaReceberId: refContaReceber.id,
      valor: Number(valor) || 0,
      data
    };
  }

  if (nome === "editar_conta_receber") {
    const { id, descricao, valor, data, senha } = input;
    if (senha !== SENHA_ALTERACAO_BANCO) {
      return { sucesso: false, erro: "senha_invalida", mensagem: "Senha incorreta. Peça a senha de autorização ao usuário para alterar o banco de dados." };
    }
    const ref = db.collection("contasReceber").doc(id);
    const doc = await ref.get();
    if (!doc.exists) return { sucesso: false, erro: "nao_encontrado", mensagem: "Lançamento não encontrado no Contas a Receber." };
  ...
  if (nome === "dar_baixa_conta_receber") {
    const { id, senha } = input;
    if (senha !== SENHA_ALTERACAO_BANCO) {
      return { sucesso: false, erro: "senha_invalida", mensagem: "Senha incorreta. Peça a senha de autorização ao usuário para alterar o banco de dados." };
    }
    ...
    const atual = doc.data();
    if (atual.status === "baixado") {
      return { sucesso: false, erro: "ja_baixado", mensagem: "Esse lançamento já estava marcado como recebido — nada foi alterado.", descricao: atual.descricao, valor: atual.valor };
    }

    const dataBaixa = new Date().toLocaleDateString("pt-BR", { timeZone: "America/Sao_Paulo" });
    await ref.update({ status: "baixado", dataBaixa });
    return { sucesso: true, id, descricao: atual.descricao, valor: atual.valor };
  }

  if (nome === "excluir_conta_receber") {
    const { id, senha } = input;
    if (senha !== SENHA_ALTERACAO_BANCO) {
      return { sucesso: false, erro: "senha_invalida", mensagem: "Senha incorreta. Peça a senha de autorização ao usuário para alterar o banco de dados." };
    }
...
async function processarMensagemAgenteGW(mensagem, historico, apiKeyValue) {
    ...
    for (let rodada = 0; rodada < 5; rodada++) {
      ...
      if (data.stop_reason === "tool_use") {
        ...
        const toolResults = await Promise.all(toolUseBlocks.map(async b => {
          ...
          let resultado;
          try {
            resultado = await executarFerramenta(b.name, b.input, apiKeyValue);
            if (alteraBanco) {
...
exports.agenteGW = onCall(
  { secrets: [anthropicApiKey, evolutionApiKey], timeoutSeconds: 120, memory: "512MiB", cors: true, invoker: "public" },
  async (request) => {
    const { mensagem, historico = [] } = request.data || {};
    if (!mensagem) throw new HttpsError("invalid-argument", "mensagem é obrigatória.");
    return processarMensagemAgenteGW(mensagem, historico, evolutionApiKey.value());
  }
);
...
Houve uma falha técnica ao processar a imagem (${e.message}). Avise o usuário que não foi possível processar o documento automaticamente, e peça pra reenviar ou informar os dados manualmente.`;
    ...
    try {
      ...
      const { resposta, historico: novoHistorico } = await processarMensagemAgenteGW(texto, historico, apiKeyValue);
...
// ── Alerta de saldo de adiantamento zerado (pedido do João, 2026-09-24) ────
// Mesmo cálculo do "Já usado essa semana" de funcionarios/app.js (limite
// semanal configurado em Configurações → Adiantamentos, mesma semana
// corrente) replicado aqui pra rodar no servidor — dispara sozinho quando um
// novo adiantamento (Contas a Pagar ou lançamento direto no Caixa) faz o
// saldo de alguém cruzar de disponível pra zerado, mesmo que ninguém esteja
// com a tela de Funcionários aberta. Duplicado de propósito (mesmo
// padrão do resto do sistema — cada tela/função tem sua cópia), mas se a
// regra de "quem bate com quem" mudar lá, replicar aqui também.
function _normNomeAdiantSrv(s) {
  return (s || "").trim().toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "");
}
function _nomeAdiantBateSrv(nome, nomeAlvo) {
  if (nome === nomeAlvo) return true;
  const palavrasNome = new Set(_normNomeAdiantSrv(nome).split(/\s+/).filter(Boolean));
  const palavrasAlvo = new Set(_normNomeAdiantSrv(nomeAlvo).split(/\s+/).filter(Boolean));
...
function _somaSeAdiantamentoDoFuncionarioSrv(descricao, criadoEm, valor, nomeAlvo, inicioSemana, cpfLancamento, cpfAlvo) {
  const desc = descricao || "";
  const m = desc.match(/^Adiantamento:?\s+(.+)/);
  if (!m) return 0;
  if (cpfLancamento) {
    ...
    if (!_nomeAdiantBateSrv(nome, nomeAlvo)) return 0;
  }
  const dt = criadoEm && criadoEm.toDate ? criadoEm.toDate() : null;
  if (!dt || dt < inicioSemana) return 0;
  return Number(valor || 0);
}
async function calcularUsadoAdiantamentoSemana(funcionario) {
  const segunda = inicioDaSemanaSrv();
  const nomeAlvo = (funcionario.nome || "").trim().normalize("NFC");
  let usado = 0;
  ...
  lancSnap.docs.forEach(d => {
    const r = d.data();
    usado += _somaSeAdiantamentoDoFuncionarioSrv(r.descricao, r.criadoEm, r.saida, nomeAlvo, segunda, r.funcionarioCpf, funcionario.cpf);
  });
  apagarSnap.docs.forEach(d => {
    const r = d.data();
    const valorReal = r.status === "baixado" ? (r.valorOriginal !== undefined ? r.valorOriginal : r.valor) : r.valor;
    usado += _somaSeAdiantamentoDoFuncionarioSrv(r.descricao, r.criadoEm, valorReal, nomeAlvo, segunda, r.funcionarioCpf, funcionario.cpf);
  });
  return usado;
}

// Roda em cima do documento recém-criado (doc já com {descricao, valor,
// funcionarioId?, funcionarioCpf?}) — só avisa quando essa criação é
// exatamente o que faz o saldo CRUZAR de disponível pra zerado (usadoAntes <
// limite <= usadoDepois), não em toda solicitação depois que já zerou.
async function avisarSeSaldoZerou(doc, apiKeyValue) {
  const nomeExtraido = extrairNomeAdiantamento(doc.descricao);
  if (!nomeExtraido) return;
  const valorEvento = Number(doc.valor || 0);
  if (valorEvento <= 0) return;

  let func = null;
  if (doc.funcionarioId) {
    const snap = await db.collection("funcionarios").doc(doc.funcionarioId).get();
    if (snap.exists) func = { id: snap.id, ...snap.data() };
  }
  if (!func) {
    const todosSnap = await db.collection("funcionarios").get();
    func = todosSnap.docs.map(d => ({ id: d.id, ...d.data() })).find(f =>
      (doc.funcionarioCpf && f.cpf && f.cpf.replace(/\D/g, "") === doc.funcionarioCpf.replace(/\D/g, "")) ||
      _nomeAdiantBateSrv(nomeExtraido, f.nome)
    );
  }
  if (!func) return;

  const cfgSnap = await db.collection("configuracoes").doc("geral").get();
  const limite = Number((cfgSnap.exists && cfgSnap.data().limiteAdiantamentoSemanal) || 0);
  if (limite <= 0) return;

  const usadoDepois = await calcularUsadoAdiantamentoSemana(func);
  const usadoAntes = usadoDepois - valorEvento;
  if (usadoAntes >= limite || usadoDepois < limite) return; // não cruzou o limite agora

  const texto = [
    "🔔 Saldo de adiantamento zerado",
    "",
    `${func.nome} atingiu o limite semanal de adiantamento (${fmtMoeda(limite)}).`
  ].join("\n");
  await enviarWhatsAppEvolution(texto, apiKeyValue, [WHATSAPP_DESTINO]);
  logger.info("[avisoSaldoAdiantamentoZerado] aviso enviado", { nome: func.nome, limite, usadoDepois });
}

exports.avisoSaldoAdiantamentoZerado = onDocumentCreated(
  { document: "contasPagar/{contaId}", secrets: [evolutionApiKey] },
  async (event) => {
    try {
      await avisarSeSaldoZerou(event.data.data(), evolutionApiKey.value());
    } catch (err) {
      logger.error("[avisoSaldoAdiantamentoZerado] falha (contasPagar)", { erro: err.message });
    }
  }
);

exports.avisoSaldoAdiantamentoZeradoCaixa = onDocumentCreated(
  { document: "lancamentos/{lancamentoId}", secrets: [evolutionApiKey] },
  async (event) => {
    const d = event.data.data();
    if (!["ANE->ADIANTAMENTO", "JOAO->ADIANTAMENTO"].includes(d.origem)) return;
    try {
      await avisarSeSaldoZerou({ ...d, valor: d.saida }, evolutionApiKey.value());
    } catch (err) {
      logger.error("[avisoSaldoAdiantamentoZerado] falha (lancamentos)", { erro: err.message });
    }
  }
);
